...
> **枝堇 Sumire 例外**：使用粉丝维护的官方 API（`https://api.sumire.live/api/stream-plans`）直接获取结构化周表数据（date/startTime/title），比 OCR 更准确、更新更及时；API 失败时自动回退 OCR。CORS 只影响浏览器端，Python 后端脚本不受影响，无需服务器转发。
...
`maintain_weekly.py`（由自动化任务每天 8 点、20 点运行）：



- 枝堇 → 拉官方 API 覆盖（每次运行，最及时）
- 本周已更新 → 前置跳过（不抓取、不重复 OCR）
